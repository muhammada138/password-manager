...
class IncrementalVault:
    ...
    def get_apps(self):
        ...
        # Pre-compute dict for O(1) lookups during sort
        order_dict = {name: i for i, name in enumerate(order)}
        return sorted(apps, key=lambda x: order_dict.get(x, len(order)))
    ...
    def get_accounts(self, app_name):
        ...
        # Pre-compute dict for O(1) lookups during sort
        order_dict = {name: i for i, name in enumerate(order)}
        return sorted(accounts, key=lambda x: order_dict.get(x, len(order)))
