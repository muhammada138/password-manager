...
class IncrementalVault:
    ...
    def get_apps(self):
        ...
        # ⚡ Bolt: Convert list to dict for O(1) lookups during sort instead of O(n) list.index()
        order_dict = {app: i for i, app in enumerate(order)}
        default_index = len(order)
        return sorted(apps, key=lambda x: order_dict.get(x, default_index))
    ...
    def get_accounts(self, app_name):
        ...
        # ⚡ Bolt: Convert list to dict for O(1) lookups during sort instead of O(n) list.index()
        order_dict = {acc: i for i, acc in enumerate(order)}
        default_index = len(order)
        return sorted(accounts, key=lambda x: order_dict.get(x, default_index))
