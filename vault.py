...
class IncrementalVault:
    ...
    def get_apps(self):
        ...
        # Optimize sorting by using a hash map to avoid O(N) index lookups
        order_map = {app: i for i, app in enumerate(order)}
        return sorted(apps, key=lambda x: order_map.get(x, len(order)))
    ...
    def get_accounts(self, app_name):
        ...
        # Optimize sorting by using a hash map to avoid O(N) index lookups
        order_map = {acc: i for i, acc in enumerate(order)}
        return sorted(accounts, key=lambda x: order_map.get(x, len(order)))
